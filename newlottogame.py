...
class Lotto:
    ...
    def check(self):
        ...
        if int(counter) == 6:
            messagebox.showinfo('result', 'You got '+str(counter)+' number(s) correct, Congratulation you won R10 000 000000')
        if int(counter) == 5:
            messagebox.showinfo('Result', 'You got '+str(counter)+' number(s) correct, You won R8,584.00')
        if int(counter) == 4:
            messagebox.showinfo('result', 'You got '+str(counter)+' number(s) correct, You won R2,384.00')
        if int(counter) == 3:
            messagebox.showinfo('Result', 'You got '+str(counter)+' number(s) correct, You won R100.50')
        if int(counter) == 2:
            messagebox.showinfo('Result', 'You got '+str(counter)+' number(s) correct, You won R20.00')
        if int(counter) == 1:
            messagebox.showinfo('Result', 'You got '+str(counter)+' number(s) correct, R0 - Better luck next time')
        if int(counter) == 0:
            messagebox.showinfo('Result', 'You got '+str(counter)+' number(s) correct, R0 - Better luck next time')
        ...
        f = open("result.txt","a+")
        text = "Date: "+date+"\nTime: "+time+"\nWinning numbers: "+str(self.numbers)+"\nUser numbers: "+str(self.userNumbers)+"\nCorrect: "+str(counter)
        f.write(text)
        f.close()
